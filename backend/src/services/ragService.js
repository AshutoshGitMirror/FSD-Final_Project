const NcertContent = require('../models/NcertContent'); // Ensure this model points to the 'ncertcontents' collection
...
async function buildContext({ std, board, subject, chapter }) {
  ...
  // Map the full chapter name to the correct chapter number format
  let mappedChapter = chapter;
  if (std && subject && chapter && CHAPTER_MAPPING[subject]?.[std]?.[chapter]) {
    mappedChapter = CHAPTER_MAPPING[subject][std][chapter];
  }
  ...
  if (board) query.board = board;
  if (subject) query.subjectName = { $regex: new RegExp('^' + esc(subject) + '$', 'i') };
  if (mappedChapter) query.chapterName = { $regex: new RegExp('^' + esc(mappedChapter) + '$', 'i') };

  try {
    let ncert = await NcertContent.findOne(query);
    if (!ncert && std && board && subject && mappedChapter) {
      const pdf = await NcertPdf.findOne({
        std: Number(std),
        board,
        subjectName: { $regex: new RegExp('^' + esc(subject) + '$', 'i') },
        chapterName: { $regex: new RegExp('^' + esc(mappedChapter) + '$', 'i') }
      });
      if (pdf?.ncertUrl) {
        const match = pdf.ncertUrl.match(/(\d{2})\.pdf$/i);
        if (match) {
