...
const SavedLinksPage = () => {
  ...
  return (
    <div className="p-8 pb-20 max-w-5xl mx-auto">
      <h1 className="text-4xl font-black uppercase mb-8 border-b-8 border-black pb-4 inline-block bg-neo-pink text-white px-4">Saved Links</h1>
      ...
      <div className="grid md:grid-cols-2 gap-6">
        ...
        {links.map((link, i) => (
          <div key={link._id || i} className="card-neo bg-white flex flex-col justify-between group hover:-translate-y-2 hover:shadow-[8px_8px_0_0_#000] p-0 overflow-hidden">
            <div className="p-6">
              <span className={`inline-block border-2 border-black font-black text-xs uppercase px-2 py-1 mb-4 text-white ${link.source === 'youtube' ? 'bg-red-500' : 'bg-blue-500'}`}>
            ...
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className={`border-t-4 border-black p-4 font-black uppercase flex justify-between items-center transition-colors ${link.source === 'youtube' ? 'bg-neo-yellow hover:bg-yellow-300' : 'bg-neo-green hover:bg-green-400'}`}
            >
