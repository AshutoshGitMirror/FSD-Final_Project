    ...
    <div className="p-8">
      <div className="flex items-center justify-between mb-6">
        ...
        {stats && (
          <div className="flex gap-3">
            <div className="card-neo bg-red-200 p-3 text-center"><span className="font-black text-xl">{stats.open}</span><p className="text-xs font-bold">Open</p></div>
            <div className="card-neo bg-green-200 p-3 text-center"><span className="font-black text-xl">{stats.reviewed}</span><p className="text-xs font-bold">Reviewed</p></div>
            <div className="card-neo bg-gray-200 p-3 text-center"><span className="font-black text-xl">{stats.total}</span><p className="text-xs font-bold">Total</p></div>
          </div>
        )}
      </div>
      ...
      <div className="flex gap-2 mb-6">
        {['open', 'reviewed', 'dismissed'].map(s => (
          <button key={s} onClick={() => setFilter(s)}
            className={`border-2 border-black px-4 py-2 font-bold uppercase text-sm transition-all ${filter === s ? 'bg-black text-white' : 'bg-white hover:bg-gray-100'}`}
          >
            {s} ({stats?.[s] || 0})
          </button>
        ))}
        <button onClick={() => setFilter('')} className="border-2 border-black px-4 py-2 font-bold uppercase text-sm bg-white hover:bg-gray-100">
          All
        </button>
      </div>
      ...
      {loadError ? (
        ...
        <div className="space-y-4">
          {flagged.map((item) => (
            <div key={item._id} className="card-neo bg-white p-6">
              <div className="flex justify-between items-start mb-3">
                <div>
                  <span className={`text-xs font-black uppercase px-2 py-1 border-2 border-black ${item.status === 'open' ? 'bg-red-200' : item.status === 'reviewed' ? 'bg-green-200' : 'bg-gray-200'}`}>
                    {item.status}
                  </span>
                  <span className="text-xs font-black uppercase px-2 py-1 border-2 border-black ml-2 bg-neo-yellow">{item.reason}</span>
                </div>
              ...
              <div className="grid grid-cols-2 gap-4 mb-4">
                <div>
                  ...
                  <p className="font-medium text-sm bg-gray-100 p-3 border-2 border-black">{item.userQuery}</p>
                </div>
                <div>
                  <p className="font-black uppercase text-xs text-gray-500 mb-1">AI Response</p>
                  <p className="font-medium text-sm bg-gray-100 p-3 border-2 border-black">{item.aiResponse}</p>
                </div>
              </div>
              {item.status === 'open' && (
                <div className="flex gap-2">
                  <button onClick={() => handleReview(item._id, 'reviewed')} className="border-2 border-black bg-green-200 px-4 py-2 font-bold text-sm uppercase hover:bg-green-300">
                    ✓ Reviewed
                  </button>
                  <button onClick={() => handleReview(item._id, 'dismissed')} className="border-2 border-black bg-gray-200 px-4 py-2 font-bold text-sm uppercase hover:bg-gray-300">
                    ✕ Dismiss
                  </button>
                </div>
              )}
