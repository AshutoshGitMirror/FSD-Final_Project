  ...
  if (error) {
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40" onClick={onClose}>
        <div className="card-neo bg-white max-w-lg w-full p-8 text-center" onClick={e => e.stopPropagation()}>
          ...
          <button onClick={onClose} className="btn-neo px-6 py-3">Close</button>
        </div>
      </div>
    );
  }
        ...
        <div className="flex items-center gap-2">
          {pdfUrl && (
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="border-2 border-black px-3 py-1 font-bold text-sm hover:bg-neo-yellow transition-colors"
            >
              Download
            </a>
          )}
          <button onClick={onClose} className="border-2 border-black px-3 py-1 font-bold text-sm hover:bg-red-200 transition-colors">
            ✕ Close
          </button>
        </div>
      </div>
      ...
      <div className="flex-1 flex items-center justify-center p-4" onClick={e => e.stopPropagation()}>
        {loading && (
          <div className="text-center text-white">
            <div className="w-12 h-12 border-4 border-white border-t-neo-yellow rounded-full animate-spin mx-auto mb-4" />
        ...
        {pdfUrl && (
          <iframe
            src={pdfUrl}
            className="w-full h-full rounded-lg shadow-[8px_8px_0_0_#000]"
